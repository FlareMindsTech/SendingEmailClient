...
function Login({ onLogin }) {
  ...
  const handleLogin = () => {
    if (email === staticCred.email && password === staticCred.password) {
      onLogin(); // Set login state to true (from App.js)
      navigate('/dashboard/user-module'); // ✅ Navigate to dashboard
    } else {
  ...
  return (
    ...
    <MDBIcon fas icon="envelope-open-text" size="2x" className="text-primary mb-3" />
    <h3 className="fw-bold">Sign in to Your Account</h3>
    <p className="text-muted">Access your admin dashboard and email services</p>
  </div>
